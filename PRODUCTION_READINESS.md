...
Sound Scheduler 1.2.0 is a **local Android sound-mode automation app**. Its stable scope includes named time routines and recurring place routines that set the phone to **Ring**, **Vibrate**, or **Silent**. Place routines operate on a user-captured local coordinate, a user-provided private label, a selected radius, and an arrival/departure transition. There are no accounts, backend services, geocoding, location sharing, analytics, or advertising SDKs.
...
| --- | --- | --- |
| Production debug compilation | Passed | `:app:assembleProdDebug` completed after the location-routine implementation. |
...
| Production lint | Passed | `:app:lintProdDebug` completed after the manifest, editor, receiver, resource, and routine-card changes. |
...
Exact-alarm and notification behavior remain separate: exact-alarm access improves time-routine precision, while notification permission affects only optional status messages. [5] [6]
...
[6] [Android Developers — Schedule exact alarms are denied by default](https://developer.android.com/about/versions/14/changes/schedule-exact-alarms)
